var CHROMEMANIFESTKEY = 'fake-location';
var EXTID_FX = '@fake-location';
var EXTID_GOOG = '';
var EXTID; // set during build
/* README
...
gulp.task('import-3rdjs', ['copy'], function() {
    ...
    var minifprod = options.production ? '.min' : '';
    return gulp.src([
			'node_modules/babel-polyfill/dist/polyfill' + minifprod + '.js',
			'node_modules/react/dist/react-with-addons' + minifprod + '.js',
			'node_modules/react-dom/dist/react-dom' + minifprod + '.js',
			'node_modules/redux/dist/redux' + minifprod + '.js',
			'node_modules/react-redux/dist/react-redux' + minifprod + '.js',
...
gulp.task('tx-js', function() {
	...
	return gulp_src_ordered(['src/**/*.js', '!src/**/3rd/*'])
        ...
        //         var includestr = '// START INCLUDE - babel-polyfill\nvar global = this;\n' + fs.readFileSync('node_modules/babel-polyfill/dist/polyfill' + (options.production ? '.min' : '') + '.js', 'utf8') + '// END INCLUDE - babel-polyfill';
